#!/bin/bash
################################################################
# Function : install_ros2_nav2_source related usage
# Desc     : 源码安装nav2导航包的脚本                        
# Platform : ubuntu                                
# Version  : 1.0                               
# Date     : Sat Dec  9 09:56:10 AM CST 2023                           
# Author   : ncnynl                             
...
echo "$(gettext "install_ros2_nav2_source related usage")"

source ${HOME}/commands/cs_utils_ros.sh

echo "This script is under DEV state !"

have_tested "[galactic,humble]"
...
function _rcm_ros_install_() {
    ...
    if [ -d ~/$workspace/src/$package_name ]; then
        ...
        git clone -b $branch  https://github.com/ros-planning/navigation2
...
function rcm_execute() {
    ...
    # start
    echo "install_ros2_nav2_source start ..."
    _rcm_ros_install_ $*
