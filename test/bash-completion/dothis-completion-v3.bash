...
_dothis_completions() {
    ...
    case "$num" in
        4)
            ...
            local prefix=${HOME}"/com/commands/$folderName/"
            ...
            case "$sugnum" in
              ...
              *)
                ...
                for i in "${!suggestions[@]}"; do
                  suggestions[$i]="$(printf '%-10s %-10s %-2s' ${items1[$i]} ${items2[$i]} -- ) ${items3[$i]}" 
                done
        ...
        3)
            ...
            sugnum=${#suggestions2[@]}
            local prefix=${HOME}"/com/commands/$folder/"
              ...
              *)
                for i in "${!suggestions2[@]}"; do
                  ...
                  if [[ $descOfScript ]]; then
                    descOfCmd="$(printf '%-10s %-10s' $scriptName --) $descOfScript" 
                  fi
                  # echo $descOfScript
        ...
        2)
            ...
            local prefix=${HOME}"/com/commands/"
              ...
              *)
                for i in "${!suggestions[@]}"; do
                  ...
                  if [[ -f $descFile ]]; then
                    ...
                    if [[ $desc ]]; then
                        descOfCmd="$(printf '%-10s %-10s' $dirName --) $desc" 
                    fi
                  else
                    descOfCmd="$(printf '%-10s %-8s %-4s\n' $dirName -- 相关功能)"
                  fi
...
# 获取目录
function subdirs() {
  echo `ls -l ~/com/commands/ | awk '/^d/ {print $NF}'`
}

# 获取脚本
function subscripts() {
  if [ -d ~/com/commands/$1 ]; then 
    echo `ls -l ~/com/commands/$1 | awk '/.sh$/ {print $NF}'`
  fi
}
